...
import itertools
#import mwrp_laze_max as laze_max
...
class Mwrp_lazy:
    ...
    #find the index for the new node in
    ...
    def get_cost(self, new_state, old_state, sort_indexing):
        ...
        cost_from_acthon_sort = [cost_from_acthon[i] for i in sort_indexing]

        cost=list(map(add, cost_from_acthon_sort, old_cost))
    ...
    # expend the best node in open list
    def expend(self):
        ...
        if old_state.first_genarate == False:
            ...
            if mtsp >= old_state.f:
                old_state.f=mtsp
                self.insert_to_open_list(old_state)
                return False
    ...
    def run(self, writer, map_config, start_pos,obs_remove):
        htype = {0: 'singlton', 1: 'max', 2: 'mtsp',3:'lazy_max',4:'BFS'}
...
class Mwrp:
    ...
    def run(self, writer, map_config, start_pos,obs_remove):
        ...
        writer.writerow([map_config, start_pos, time() - self.start_time, h_type[self.index], self.H_start,
                         h_gen,
                        h_exp, self.max_pivot, 0, self.genrate_node, self.expend_node,
                         self.open_is_beter,self.new_is_beter,obs_remove, goal_node.cost])
                    ...
                    for start_pos in all_start_config_as_tupel:
                        ...
                        for huristic in huristics_exp:
                            if exp_index >= start_in:
                                ...
                                if huristic==3:
                                    mwrp = Mwrp_lazy(world, start_pos, huristic, max_pivot,map_type,minimize['soc'])
                                else:
